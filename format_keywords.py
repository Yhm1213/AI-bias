...
# Build a dictionary mapping each ID to its standard (Chinese, English)
# Note: I'll manually seed the dictionary from the previous grep output.
dict_map = {
    "温柔": ("温柔", "Gentle"),
    "装饰": ("装饰", "Decorative"),
    "高大": ("高大", "Tall"),
...
# The files has 4 sections: 
# CN_GENDER_BIAS_DATA_CN (CN format)
# CN_GENDER_BIAS_DATA_EN (EN format)
# EN_GENDER_BIAS_DATA_CN (CN format)
# EN_GENDER_BIAS_DATA_EN (EN format)
# The transition between these sections can be found by looking for the array names.
...
for line in content.split('\n'):
    ...
    # regex to match <Keyword id="xxx" color="xxx">TEXT</Keyword>
    def replacer(match):
        ...
        # Determine Chinese and English for this id
        zh, en = dict_map.get(kw_id, (kw_id, kw_id))
        
        # Format the bilingual text
        if current_lang == 'CN':
            bilingual_text = f"{zh}（{en}）"
        else:
            # Capitalize the first letter of English if it's the beginning
            en_cap = en[0].upper() + en[1:] if en else en
            bilingual_text = f"{en_cap} ({zh})"
            
        # Try to extract the count from the inner_text
        count_str = ""
        # Chinese format like （XX次）, （XX次 vs XX次）, ，XX次
        # English format like (XX times), (XX vs. XX times)
        
        # Check for count patterns
        # 1. Chinese format inside brackets: （...次）
        m1 = re.search(r'（(.*?[次times].*?)）', inner_text, re.IGNORECASE)
        # 2. Chinese format with comma: ，...次
        m2 = re.search(r'，(.*?[次times].*)', inner_text, re.IGNORECASE)
        # 3. English format inside parentheses: (XX times)
        m3 = re.search(r'\((.*?[次times].*?)\)', inner_text, re.IGNORECASE)
