...
# Add language props
content = content.replace("interface DataExplanationProps {", "interface DataExplanationProps {\n  language: 'CN' | 'EN';\n  toggleLanguage: () => void;")
content = content.replace("const DataExplanation: React.FC<DataExplanationProps> = ({ onBack, highlightId }) => {", "const DataExplanation: React.FC<DataExplanationProps> = ({ onBack, highlightId, language, toggleLanguage }) => {")

# Update title
content = content.replace("数据说明", "{language === 'EN' ? 'Data Explanation' : '数据说明'}")

# Generate the items array for both languages
items_logic = """
...
# Replace items array
content = re.sub(r'const items: DataExplanationItem\[\] = \[.*?\];', items_logic, content, flags=re.DOTALL)

# Add language toggle button at the bottom right, similar to DiscoverySlides.
language_btn_code = """
...
content = content.replace("</button>\n    </div>", "</button>\n    </div>" + language_btn_code)

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)
print('Updated DataExplanation.tsx with language props and translation')
